...
# Apenas o cliente do Tasks é inicializado globalmente
...
def vivi_webhook(request):
    ...
    if tag == 'identificar_cliente':
        ...
        if not all([service_url, project_id, location_id, queue_id]):
            print("❌ ERRO FATAL: Uma ou mais variáveis de ambiente (SERVICE_URL, GCP_PROJECT_ID, etc.) não foram encontradas.")
            texto_resposta = "Ocorreu um erro interno de configuração. Nossa equipe foi notificada."
        else:
